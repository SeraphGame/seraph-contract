...
ReentrancyGuardUpgradeable
{
    ...
    bytes32 public constant BUY_MINTER_ROLE = keccak256("BUY_MINTER_ROLE");
    ...
    ) external onlyAdmin(GAME_MINTER_ROLE) nonReentrant {
        require(tokenId > 9999, "tokenId is too small");
        _commonMint(to, tokenId, content);
    }
    ...
    function buyMint(
        ...
        uint256 tokenId,
        string calldata content
    ) external onlyAdmin(BUY_MINTER_ROLE) nonReentrant {
        require(tokenId >= 1000, "tokenId is too small");
        require(tokenId <= 9999, "tokenId is too big");
        _commonMint(to, tokenId, content);
    }
    ...
    function adminMint(
        ...
        uint256 tokenId,
        string calldata content
    ) external onlyAdmin(ADMIN_MINTER_ROLE) nonReentrant {
        require(tokenId <= 999, "tokenId is too big");
        _commonMint(to, tokenId, content);
    }
    ...
    function batchMint(
        ...
        string[] calldata content
    ) public onlyAdmin(ADMIN_MINTER_ROLE) nonReentrant {
        require(
